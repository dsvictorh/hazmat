...
angular.module('ngWYSIWYG').directive('ngpContentFrame', ['ngpImageResizer', 'ngpUtils', 'NGP_EVENTS', '$compile',
	'$timeout', '$sanitize', function (ngpImageResizer, ngpUtils, NGP_EVENTS, $compile, $timeout, $sanitize) {
	    ...
	    var linker = function (scope, $element, attrs, ctrl) {
	        ...
	        $document.write('<!DOCTYPE html><html><head><link rel="stylesheet" href="/assets/less/modules/hazmat/wysiwyg-iframe.css" /><></head><body contenteditable="true"></body></html>');
	        $document.close();
	        ...
	        $body.keydown(function (e) {
	            ...
	            if (e.keyCode === 13) {
	                ...
	                if (startContainer.nodeName !== 'LI' && startContainer.parentElement.nodeName !== 'LI' && endContainer.nodeName !== 'LI' && endContainer.parentElement.nodeName !== 'LI') {
	                    scope.textStyle = '<p>';
	                    $timeout(function () {
	                        $document.execCommand('formatblock', false, scope.textStyle);
	                    }, 1);
	                }
	            }
	        });
...
var editorTemplate = "<div class=\"tinyeditor\">" +
	"<div class=\"tinyeditor-header\" ng-hide=\"editMode && dev\">" +
	"{toolbar}" + // <-- we gonna replace it with the configured toolbar
	"<div style=\"clear: both;\"></div>" +
	"</div>" +
	"<div class=\"sizer\"  ngp-resizable>" +
	"<textarea ng-if=\"dev\" data-placeholder-attr=\"\" style=\"-webkit-box-sizing: border-box; -moz-box-sizing: border-box; box-sizing: border-box; resize: none; width: 100%; height: 100%;\" ng-show=\"editMode\" ng-model=\"content\"></textarea>" +
	"<iframe style=\"-webkit-box-sizing: border-box; -moz-box-sizing: border-box; box-sizing: border-box; width: 100%; height: 100%;\" ng-hide=\"editMode && dev\" ngp-content-frame=\"{sanitize: config.sanitize}\" text-style=\"textstyle\" content-style=\"{contentStyle}\" ng-model=\"content\"></iframe>" +
	"</div>" +
	"<div ng-show=\"dev\" class=\"tinyeditor-footer\">" +
	...
	function (ngpUtils, NGP_EVENTS, $rootScope, $compile, $timeout, $q) {
	    var linker = function (scope, $element, attrs, ctrl) {
	        ...
	        scope.toolbar = (scope.config && scope.config.toolbar) ? scope.config.toolbar : [
				{ name: 'basicStyling', items: ['bold', 'italic', 'underline', 'strikethrough', 'subscript', 'superscript', scope.restrictToParagraph ? "-" : "leftAlign", scope.restrictToParagraph ? "-" : "centerAlign", scope.restrictToParagraph ? "-" : "rightAlign", scope.restrictToParagraph ? "-" : "blockJustify", '-'] },
				{ name: 'paragraph', items: scope.restrictToParagraph ? [] : ["orderedList", "unorderedList", "outdent", "indent", "-"] },
				{ name: 'doers', items: ['removeFormatting', 'undo', 'redo', '-'] },
				{ name: 'colors', items: scope.showColor ? ["fontColor", "backgroundColor", "-"] : [] },
				{ name: 'links', items: [scope.restrictToParagraph ? "-" : "hr", "symbols", "link", "unlink", scope.showImage ? "image" : "-", "-"] },
				{ name: 'tools', items: scope.showPrint ? ["print", "-"] : [] },
				{ name: 'styling', items: scope.restrictToParagraph ? [] : ["format"] }
	        ];
			 ...
			 */
	        scope.styles = [{ name: 'Paragraph', key: '<p>' }, { name: 'Header 3', key: '<h3>' }, { name: 'Header 4', key: '<h4>' }, { name: 'Header 5', key: '<h5>' }, { name: 'Header 6', key: '<h6>' }];
	        ...
	        scope.$watch('content', function (newValue) {
	            if (!newValue) {
	                ...
	                scope.content = newValue.replace(/<div>/g, '<p>').replace(/<\/div>/g, '</p>').replace(/<p><ul>/g, '<ul>').replace(/<\/p><\/ul>/g, '</ul>');
	            }
	        });
